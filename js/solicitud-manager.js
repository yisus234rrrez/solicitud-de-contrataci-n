...
class SolicitudManager {
     ...
     * En lugar de poner un onchange a cada select, se escucha en toda la tabla (Event Delegation).
     * Esto mejora el rendimiento y funciona para filas nuevas creadas dinámicamente.
     */
    setupEventListeners() {
        this.tbody.addEventListener('change', (e) => {
            ...
            // Busca automáticamente el código correcto en los diccionarios de la clase
            // y lo inyecta en el input "readonly" correspondiente al lado del select.
            ...
            if (target.name === 'funcion_nombres[]') {
                const codeInput = tr.querySelector('[name="funcion_codigos[]"]');
                if (codeInput) codeInput.value = this.FUNCION_DATA[target.value] || '';
            }
        });
    }
    ...
    addRow() {
        ...
        const funcionOptions = this._buildOptions(this.FUNCION_DATA, 'Seleccione Función...');
        ...
        tr.innerHTML = `
            ...
            <td class="px-3 py-2 border-r border-gray-100">
                <select name="funcion_nombres[]" class="${this.inputClass}">${funcionOptions}</select>
            </td>
            <td class="px-3 py-2 border-r border-gray-100"><input type="text" name="funcion_codigos[]" placeholder="Cód. Func." class="${this.inputClass} text-center" readonly></td>
...
// Instancia global de la clase Manager.
// Se ancla al objeto `window` para que pueda ser invocada desde cualquier lugar,
// incluyendo el atributo `onclick` que podría encontrarse en el HTML.
window.solicitudManager = new SolicitudManager();

// Interfaz global simplificada (wrapper) para el botón del HTML.
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Wire up the button explicitly to make sure it functions properly
